...
public class StreamMatch {
	public static void main(String[] args) {
		int[]  digits = { 1, 100, 35, 1, 99, 5, 8, 10, 77, 12 };
...
//		2nd: find the first element that is divisible by 4
		// find first
		OptionalInt ex2 = Arrays.stream(digits) // int stream
			.filter(digit -> digit % 4 == 0)
			.findFirst();
		if(ex2.isPresent()) {
			System.out.println("VALUE: " + ex2.getAsInt());
		}
		
//		3nd: find the element that is divisible by 5 in the list
		// findAny
		OptionalInt ex3 = Arrays.stream(digits)
				.filter(digit -> digit % 5 == 0)
				.findAny();
		if(ex3.isPresent()) {
			System.out.println("optInt3: " + ex3.getAsInt());
		}
...
//		5th: does no elements greater than 0
		// noneMatch
		boolean noGreaterThan0 = Arrays.stream(digits)
				.noneMatch(digit -> digit > 0);
				System.out.println("noGreaterThan0: " + noGreaterThan0);
	}
}
