...
public class Ex01 {
	public static void main(String[] args) {
		...
		transactions.stream().filter(t -> t.getValue() > 300).sorted(Comparator.comparing(t -> t.getTrader().getCity()))
				.forEach(System.out::println);
		...
		transactions.stream().filter(t -> "Cambridge".equals(t.getTrader().getCity()))
				...
				.map(t -> t.getTrader().getName()).distinct().forEach(System.out::println);
		...
		long count = transactions.stream().filter(t -> "Milan".equals(t.getTrader().getCity()))
				.map(t -> t.getTrader().getName()).distinct().count();
		...
		int minValue = transactions.stream().mapToInt(Transaction::getValue).min().getAsInt();
		transactions.stream().filter(t -> t.getValue() == minValue).forEach(System.out::println);
