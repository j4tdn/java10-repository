...
public class SuspendResumeDeadlock {
	...
	private static String sharedObject = "abcxyz";
	...
	public static void main(String[] args) throws InterruptedException {
			...
			public void run() {
				...
				synchronized (sharedObject) {
					System.out.println(Thread.currentThread().getName() + " " + " has obtained lock on sharedObject "
							+ " & suspended...");
					/*
					 * suspend the thread, now this thread will release lock on sharedObject (by
		...
		Thread thread2 = new Thread("Thread-2") {
			public void run() {
				...
				/*
				 * for acquiring lock on sharedObject thread-1 must have released lock on
