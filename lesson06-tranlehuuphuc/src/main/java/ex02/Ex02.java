...
private static Scanner sc = new Scanner(System.in);
	...
	private static String checkUserName() {
		...
		if (userName.isBlank()) {
			System.err.println("UserName empty!");
			System.out.print("Enter again: ");
			return checkUserName();
		}
		if (Character.isDigit(userName.charAt(0))) {
			System.err.println("Username must not start with a number!");
			System.out.print("Enter again: ");
			return checkUserName();
		}
		return userName;
	...
	private static String checkPassword() {
		...
		if (password.length() < 8 || password.length() > 256) {
			System.err.println("Password length must be at least 8 characters and not more than 256 characters!");
			System.out.print("Enter again: ");
			return checkPassword();
		}
		if (!checkPass(password, c -> Character.isLowerCase(c))) {
			System.err.println("Password must contain at least 1 lowercase character!");
			System.out.print("Enter again: ");
			return checkPassword();
		}
		if (!checkPass(password, c -> Character.isUpperCase(c))) {
			System.err.println("Password must contain at least 1 uppercase character!");
			System.out.print("Enter again: ");
			return checkPassword();
		}
		if (!checkPass(password, c -> Character.isDigit(c))) {
			System.err.println("Password must contain at least 1 number!");
			System.out.print("Enter again: ");
			return checkPassword();
		}
